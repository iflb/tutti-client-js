...
class Duct extends ducts.Duct {
    ...
    setupHandlers(self) {
        ...
        self.setEventHandler( self.EVENT.MTURK_SEND_BONUS,
                              (rid, eid, data) => { self._handleMTurk("sendBonus", data); } );
    }
}
...
class MTurkController extends TuttiController {
    constructor( duct ){
        ...
        this.sendBonus =
            ( WorkerIds, AssignmentIds, BonusAmount, Reason ) => {
                return this._duct.send( this._duct.nextRid(), this._duct.EVENT.MTURK_SEND_BONUS, this._data({ WorkerIds, AssignmentIds, BonusAmount, Reason }) );
            };
    }
}
