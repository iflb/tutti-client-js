...
class ResourceController extends TuttiController {
    constructor(duct){
        ...
        this._callOrSend = (eid, { required: requiredKeys = [], optional: optionalKeys = [] }, args = {}) => {
                const send = (eid, args) => {
                ...
                const unknown = Object.keys(args).filter((key) => (!requiredKeys.includes(key) && !optionalKeys.includes(key) && key!=='awaited'));
                if(unknown.length>0) console.warn(`Unknown parameter(s): ${unknown}`);

                let { awaited, access_token, ..._args } = args;
                if(access_token) _args.access_token = access_token;
        ...
        this._methods = {
            'signUp':
                (args) => {
                    if('password' in args) { args.password_hash = crypto.createHash('md5').update(args.password, 'binary').digest('hex'); delete args.password; }
                    return this._callOrSend(this._duct.EVENT.AUTHENTICATION_SIGN_UP, { required: ['user_name', 'password_hash', 'privilege_ids'] }, args);
                },

            'signIn':
                (args) => {
                    if('password' in args) { args.password_hash = crypto.createHash('md5').update(args.password, 'binary').digest('hex'); delete args.password; }
                    return this._callOrSend(this._duct.EVENT.AUTHENTICATION_SIGN_IN, { optional: ['user_name', 'password_hash', 'access_token'] }, args);
                },

            'signOut':
                (args) => this._callOrSend(this._duct.EVENT.AUTHENTICATION_SIGN_OUT, { required: ['access_token'] }, args),

            'getUserIds':
                (args) => this._callOrSend(this._duct.EVENT.ACCOUNT_LIST_IDS, { optional: ['access_token'] }, args),

            'associateUserPrivilege':
                (args) => this._callOrSend(this._duct.EVENT.ACCOUNT_ASSOCIATE_USER_PRIVILEGE, ['user_name'], args),

            'unassociateUserPrivilege':
                (args) => this._callOrSend(this._duct.EVENT.ACCOUNT_UNASSOCIATE_USER_PRIVILEGE, ['user_name', 'privilege'], args),


            'getEventHistory':
                (args) => this._callOrSend(this._duct.EVENT.EVENT_HISTORY, [], args),

            'setEventHistory':
                (args) => this._callOrSend(this._duct.EVENT.EVENT_HISTORY, ['eid', 'query'], args),

            'getProjectScheme':
                (args) => this._callOrSend(this._duct.EVENT.PROJECT_GET_SCHEME, ['project_name', 'cached'], args),


            'getWorkerIdForPlatform':
                (args) => this._callOrSend(this._duct.EVENT.WORKER_GET_ID_FOR_PLATFORM, ['platform', 'platform_worker_id'], args),

            'getWorkerIdsForProject':
                (args) => this._callOrSend(this._duct.EVENT.WORKER_GET_IDS_FOR_PROJECT, ['project_name'], args),


            'createSession':
                (args) => this._callOrSend(this._duct.EVENT.SESSION, ['command', 'project_name', 'platform_worker_id', 'client_token', 'platform', 'nanotask_group_ids'], { command: 'create', ...args }),


            'getNanotaskGroup':
                (args) => this._callOrSend(this._duct.EVENT.NANOTASK_GROUP_GET, ['nanotask_group_id'], args),

            'listNanotaskGroups':
                (args) => this._callOrSend(this._duct.EVENT.NANOTASK_GROUP_LIST, ['project_name', 'template_name'], args),

            'addNanotaskGroup':
                (args) => this._callOrSend(this._duct.EVENT.NANOTASK_GROUP_ADD, ['name', 'nanotask_ids', 'project_name', 'template_name'], args),

            'deleteNanotaskGroup':
                (args) => this._callOrSend(this._duct.EVENT.NANOTASK_GROUP_DELETE, ['nanotask_group_id'], args),


            'uploadNanotasks':
                (args) => this._callOrSend(this._duct.EVENT.NANOTASK_UPLOAD, ['projectName', 'templateName', 'nanotasks', 'numAssignable', 'priority', 'tagName'], args),


            'getAutomationParameterSet':
                (args) => this._callOrSend(this._duct.EVENT.AUTOMATION_PARAMETER_SET_GET, ['automation_parameter_set_id'], args),

            'listAutomationParameterSets':
                (args) => this._callOrSend(this._duct.EVENT.AUTOMATION_PARAMETER_SET_LIST, [], args),

            'createAutomationParameterSet':
                (args) => this._callOrSend(this._duct.EVENT.AUTOMATION_PARAMETER_SET_ADD, ['name', 'platform_parameter_set_id', 'project_name'], args),


            'getPlatformParameterSet':
                (args) => this._callOrSend(this._duct.EVENT.PLATFORM_PARAMETER_SET_GET, ['platform_parameter_set_id'], args),

            'listPlatformParameterSets':
                (args) => this._callOrSend(this._duct.EVENT.PLATFORM_PARAMETER_SET_LIST, [], args),

            'createPlatformParameterSet':
                (args) => this._callOrSend(this._duct.EVENT.PLATFORM_PARAMETER_SET_ADD, ['name', 'platform', 'parameters'], args),


            'executeAutomation':
                (args) => this._callOrSend(this._duct.EVENT.EXECUTE_AUTOMATION, ['automation_parameter_set_id', 'parameters'], args),
        };
        this.loadMethods();
    ...
    loadMethods() {
        Object.entries(this._methods).forEach(([name, f]) => {
            ...
            this[name] = (args = {}) => f({ access_token, ...args, awaited: !('awaited' in args) || args.awaited });
            this[name].call = (args = {}) => f({ access_token, ...args, awaited: true });
            this[name].send = (args = {}) => f({ access_token, ...args, awaited: false });
        });
    }
