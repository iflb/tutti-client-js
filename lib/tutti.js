...
class ResourceEventListener extends DuctEventListener {
    ...
    bindTuttiHandlersToDuct(self) {
        ...
        set(self._duct.EVENT.PROJECT_ADD,
                (rid, eid, data) => { _hr("createProject", rid, data); } );
        ...
        set(self._duct.EVENT.NANOTASK_ADD_MULTI_FOR_TEMPLATE,
                (rid, eid, data) => { _hr("createNanotasks", rid, data); } );
...
class MTurkEventListener extends DuctEventListener {
    ...
    bindTuttiHandlersToDuct(self) {
        ...
        set(self._duct.EVENT.MARKETPLACE_MTURK_LIST_CREDENTIALS,
        ...
        set(self._duct.EVENT.MARKETPLACE_MTURK_TUTTI_HIT_BATCH_LIST_WITH_HITS,
                (rid, eid, data) => { _hm("listTuttiHITBatchesWithHITs", rid, data); } );
        ...
        set(self._duct.EVENT.MARKETPLACE_MTURK_TUTTI_HIT_BATCH_DELETE,
                (rid, eid, data) => { _hm("deleteTuttiHITBatch", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_HIT_TYPE_LIST,
                (rid, eid, data) => { _hm("listHITTypes", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_QUALIFICATION_TYPE_LIST,
                (rid, eid, data) => { _hm("listQualificationTypes", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_QUALIFICATION_TYPE_CREATE,
                (rid, eid, data) => { _hm("createQualificationType", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_QUALIFICATION_TYPE_DELETE,
                (rid, eid, data) => { _hm("deleteQualificationTypes", rid, data); } );
        ...
        set(self._duct.EVENT.MARKETPLACE_MTURK_HIT_EXPIRE,
                (rid, eid, data) => { _hm("expireHITs", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_HIT_DELETE,
                (rid, eid, data) => { _hm("deleteHITs", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_HIT_LIST_FOR_TUTTI_HIT_BATCH,
                (rid, eid, data) => { _hm("listHITsForTuttiHITBatch", rid, data); } );
        //set(self._duct.EVENT.MTURK_CREATE_QUALIFICATION,
        //        (rid, eid, data) => { _hmo("createQualification", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_WORKER_LIST,
                (rid, eid, data) => { _hm("listWorkers", rid, data); } );
        ...
        set(self._duct.EVENT.MARKETPLACE_MTURK_WORKER_NOTIFY,
                (rid, eid, data) => { _hm("notifyWorkers", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_WORKER_ASSOCIATE_QUALIFICATIONS,
                (rid, eid, data) => { _hm("associateQualificationsWithWorkers", rid, data); } );
        //set(self._duct.EVENT.MTURK_LIST_ASSIGNMENTS,
        //        (rid, eid, data) => { _hmo("listAssignments", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_ASSIGNMENT_LIST_FOR_TUTTI_HIT_BATCH,
                (rid, eid, data) => { _hm("listAssignmentsForTuttiHITBatch", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_ASSIGNMENT_APPROVE,
                (rid, eid, data) => { _hm("approveAssignments", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_ASSIGNMENT_REJECT,
                (rid, eid, data) => { _hm("rejectAssignments", rid, data); } );
        set(self._duct.EVENT.MARKETPLACE_MTURK_ASSIGNMENT_SEND_BONUS,
                (rid, eid, data) => { _hm("sendBonus", rid, data); } );
    ...
    constructor(duct){
        ...
        this._setMethods({
            ...
            expireHITs({ request_id, hit_ids }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_HIT_EXPIRE, { request_id, hit_ids }, arguments);
                },
            deleteHITs({ request_id, hit_ids }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_HIT_DELETE, { request_id, hit_ids }, arguments);
                },
            listHITsForTuttiHITBatch({ batch_id, cached }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_HIT_LIST_FOR_TUTTI_HIT_BATCH, { batch_id, cached }, arguments);
                },
            listTuttiHITBatches() {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_TUTTI_HIT_BATCH_LIST, {}, arguments);
                },
            listTuttiHITBatchesWithHITs() {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_TUTTI_HIT_BATCH_LIST_WITH_HITS, {}, arguments);
                },
            ...
            deleteTuttiHITBatch({ request_id, batch_id }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_TUTTI_HIT_BATCH_DELETE, { request_id, batch_id }, arguments);
                },
            listHITTypes() {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_HIT_TYPE_LIST, {}, arguments);
                },
            listQualificationTypes({ query, only_user_defined, cached }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_QUALIFICATION_TYPE_LIST, { query, only_user_defined, cached }, arguments);
                },
            createQualificationType({ name, description, auto_granted, qualification_type_status }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_QUALIFICATION_TYPE_CREATE, { name, description, auto_granted, qualification_type_status }, arguments);
                },
            deleteQualificationTypes({ qualification_type_ids }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_QUALIFICATION_TYPE_DELETE, { qualification_type_ids }, arguments);
                },
            listWorkers() {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_WORKER_LIST, {}, arguments);
                },
            notifyWorkers({ subject, message_text, worker_ids }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_WORKER_NOTIFY, { subject, message_text, worker_ids }, arguments);
                },
            associateQualificationsWithWorkers({ qualification_type_id, worker_ids, integer_value, send_notification }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_WORKER_ASSOCIATE_QUALIFICATIONS, { qualification_type_id, worker_ids, integer_value, send_notification }, arguments);
                },
            listAssignmentsForTuttiHITBatch({ batch_id, cached }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_ASSIGNMENT_LIST_FOR_TUTTI_HIT_BATCH, { batch_id, cached }, arguments);
                },
            approveAssignments({ assignment_ids, requester_feedback, override_rejection }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_ASSIGNMENT_APPROVE, { assignment_ids, requester_feedback, override_rejection }, arguments);
                },
            rejectAssignments({ assignment_ids, requester_feedback }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_ASSIGNMENT_REJECT, { assignment_ids, requester_feedback }, arguments);
                },
            sendBonus({ worker_ids, bonus_amount, assignment_ids, reason }) {
                    return self._callOrSend(self._duct.EVENT.MARKETPLACE_MTURK_ASSIGNMENT_SEND_BONUS, { worker_ids, bonus_amount, assignment_ids, reason }, arguments);
                },
        });
    }
}
    ...
    constructor(duct){
        ...
        this._setMethods({
            ...
            createNanotasks({ id, project_name, template_name, nanotasks, tag, priority, num_assignable }) {
                    return self._callOrSend(self._duct.EVENT.NANOTASK_ADD_MULTI_FOR_TEMPLATE, { id, project_name, template_name, nanotasks, tag, priority, num_assignable }, arguments);
                },
