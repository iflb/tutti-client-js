...
class ResourceController extends TuttiController {
    constructor(duct){
        ...
        this._callOrSend = (eid, keys = [], args = {}) => {
                const send = (eid, args) => {
                ...
                const unknown = Object.keys(args).filter((key) => (!keys.includes(key) && key!=='awaited'));
                if(unknown.length>0) console.warn(`Unknown parameter(s): ${unknown}`);
                //const _args = keys.reduce((a, c) => ((c in args) ? { ...a, [c]: args[c] } : a), {});
                const { awaited, ..._args } = args;
        ...
        this._methods = {
            ...
            'getRequestParameterSet':
                (args) => this._callOrSend(this._duct.EVENT.REQUEST_PARAMETER_SET_GET, ['request_parameter_set_id'], args),

            'listRequestParameterSets':
                (args) => this._callOrSend(this._duct.EVENT.REQUEST_PARAMETER_SET_LIST, [], args),

            'createRequestParameterSet':
                (args) => this._callOrSend(this._duct.EVENT.REQUEST_PARAMETER_SET_ADD, ['name', 'platform_parameter_set_id', 'project_name'], args),
            ...
            'request':
                (args) => this._callOrSend(this._duct.EVENT.REQUEST, ['request_parameter_set_id', 'nanotasks', 'nanotask_group_name', 'job_description'], args),
        };
        this.loadMethods();
    ...
    loadMethods() {
        Object.entries(this._methods).forEach(([name, f]) => {
            this[name] = (args = {}) => f({ ...args, awaited: !('awaited' in args) || args.awaited });
            this[name].call = (args = {}) => f({ ...args, awaited: true });
            this[name].send = (args = {}) => f({ ...args, awaited: false });
        });
    }
